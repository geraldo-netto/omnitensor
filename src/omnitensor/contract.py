...
is broken" — a ``method-unknown`` error, or a refusal code — which is the wrong
diagnosis to show a user and the wrong one to act on.  Enumerating method names
would not answer it either: a name says nothing about which version of the
document body behind it the service accepts.  Two services can both export
``submit-job`` and disagree completely about what a submission looks like.
...
The lists are not written here by hand.  ``methods`` is the table the socket
transport asserts its dispatch against at startup and ``schemas`` is derived
from the shipped schema files, so a method or a contract added on one side
cannot quietly go unannounced on the other.
"""
...
# Only the documents that cross the control socket.  A workload manifest and a
# plugin manifest are read from disk by the service itself; announcing them
# would describe an agreement this handshake is not part of.
_WIRE_SCHEMA_PREFIXES = ("control-", "runtime-", "plugin-inventory")


def wire_schema_names(names: Iterable[str]) -> tuple[str, ...]:
    """The schema files that describe documents crossing the socket."""
