...
so it is not in the wheel installed above. Install it in a producer venv with
its `[train]` extra, add `[convert]` for ncnn GPU artifacts and/or
`[convert-npu]` for OpenVINO NPU artifacts, then install only the generated
native files into the service artifact store. See
