...
| `document-intelligence` | disabled, weight 2 | Document classification, similarity, or layout detection | No large weights are bundled. BGE has an explicit one-shot producer that fetches pinned MIT sources, builds and gates a digest-locked ncnn embedding artifact on a named Vulkan GPU, and installs a disabled local binding. MiniLM and BGE remain reviewed portable sources; NPU/TPU ports, private-domain quality, selected-file indexing/answers, and full acceptance remain. Exact duplicates use hashing. |
